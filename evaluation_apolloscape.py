...
if __name__ == "__main__":
    ...
    for index in range(len(filelist)):
        ...
        if opt.kitti2015:
            ...
            ## Here is the code to evaluate Apolloscape dataset
        else:
            leftname = opt.data_path + 'stereo_test/camera_5/' + current_file[0: len(current_file) - 6] + '5.jpg'
            rightname = opt.data_path + 'stereo_test/camera_6/' + current_file[0: len(current_file) - 6] + '6.jpg'
            dispname = opt.data_path + 'stereo_test/disparity/' + current_file[0: len(current_file) - 1]
            savename = opt.save_path + current_file[0: len(current_file) - 1]
            disp = Image.open(dispname)
            disp = np.asarray(disp) / 256.0
            # disp, height, width = readPFM(dispname)

        ## Here is the original code to evaluate scene flow dataset
        '''
        else:
        ...
        ## Crop gt disparity image
        _, h_d, w_d=np.shape(disp)
